...
const AuthorsGallery = () => {
  ...
  const fetchAuthors = async () => {
    setLoading(true);
    setError(false);
    const authors = await BooksService.getAuthors().then((data) => {
      console.log(data);
  ...
  if(error) {
    return (
      <ErrorComponent onPress={fetchAuthors} />
    )
  }

  return (
    <div className="p-8  from-gray-50 to-gray-100">
      <h2 className="text-3xl font-bold  mb-8 text-center">Nos Auteurs</h2>
